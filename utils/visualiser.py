...
import ntpath
import time
from utils import util, my_html as html
...
class Visualiser():
    ...
    def plot_table_html(self, x, y, key, split_name, **kwargs):
        ...
        table_html = table.round(2).to_html(
            col_space=200, bold_rows=True, border=12)
    ...
    def plot_line(self, x, y, key, split_name):
        if key not in self.error_plots:
            ...
            self.vis.line(X=np.array([x]), Y=np.array(
                [y]), win=self.error_plots[key], name=split_name, update='append')
