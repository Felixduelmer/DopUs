...
class DopUsV1(nn.Module):
    ...
    def __init__(self, feature_scale=8, n_classes=1, is_deconv=True, in_channels=2, is_batchnorm=True):
        ...
        self.up_concat4 = unetUp2(filters[4]*2, filters[3], self.is_deconv, self.is_batchnorm)
        self.up_concat3 = unetUp2(filters[3]*2, filters[2], self.is_deconv, self.is_batchnorm)
        self.up_concat2 = unetUp2(filters[2]*2, filters[1], self.is_deconv, self.is_batchnorm)
        self.up_concat1 = unetUp2(filters[1]*2, filters[0], self.is_deconv, self.is_batchnorm)
    ...
    def forward(self, inputs, hidden):
        ...
        up4 = self.up_concat4(torch.cat((conv4_doppler, conv4_bmode), 1), center)
        up3 = self.up_concat3(torch.cat((conv3_doppler, conv3_bmode), 1), up4)
        up2 = self.up_concat2(torch.cat((conv2_doppler, conv2_bmode), 1), up3)
        up1 = self.up_concat1((torch.cat(conv1_doppler, conv1_bmode), 1), up2)
